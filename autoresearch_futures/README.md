...
# 实时监控 (交易时段轮询K线并生成信号)
python -m autoresearch_futures monitor --symbols rb i
...
`monitor` 命令在交易时段自动轮询 K 线数据，使用优化后的策略参数生成交易信号：

```bash
# 默认监控 rb 和 i 品种
python -m autoresearch_futures monitor

# 监控指定品种
python -m autoresearch_futures monitor --symbols rb i au cu
```

**交易时段**:
- 日盘: 9:00 - 11:30
- 下午盘: 13:30 - 15:00
- 夜盘: 21:00 - 23:00 (部分品种至次日 01:00 或 02:30)
